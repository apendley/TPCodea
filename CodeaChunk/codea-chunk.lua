...
return {
	...
	frames={{% for sprite in allSprites %}
		["{{sprite.trimmedName}}"] = {
			...
			frameSize={ w = {{sprite.frameRect.width}}, h = {{sprite.frameRect.height}} },
			uvRect = { s = {{sprite.frameRect.x}}/{{texture.size.width}}, t = 1-(({{sprite.frameRect.y}}+{{sprite.frameRect.height}})/{{texture.size.height}}), tw = {{sprite.frameRect.width}}/{{texture.size.width}}, th = {{sprite.frameRect.height}}/{{texture.size.height}} }
		},{% endfor %}
	}
}